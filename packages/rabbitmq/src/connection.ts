...
export class RabbitMQConnection {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      this.connection = (await amqp.connect(connectionString)) as Connection;
      if (!this.connection) {
      ...
      this.isConnecting = false;
      systemLogger.error('Failed to connect to RabbitMQ', { error: error.message });
  ...
  async disconnect(): Promise<void> {
    try {
      ...
      systemLogger.error('Error disconnecting from RabbitMQ', { error: error.message });
    }
  }
  ...
  private async handleReconnect(): Promise<void> {
    ...
    setTimeout(async () => {
      try {
        ...
        systemLogger.error('Reconnection failed', { error: error.message });
      }
    }, delay);
  }
}
